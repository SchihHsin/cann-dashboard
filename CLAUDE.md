...
- **组件视图** `#view-component`：综览（同款 `.jkpi` 卡，11 个组件口径）/ 组件体验矩阵（`#compHeatGrid`，**行=11 个组件（算子开发 / Ascend C / 图引擎 / 运行时 / 算子编程-PyPTO / 算子库 / 通信库-集合通信 / 通信库-单边通信 / 工具 / PTO / 编译库&OS），列=文档/API/工具/环境 + 综合**，数据 `touchpointData`，cells 为 `{s,t}`（模拟，无 obs），`renderCompMatrix`/`selectCompCell` 点格 → `#compDetail`；行支持排序 `setCompSort('alpha'|'score')`——**默认字母排序**（`sceneName.localeCompare(_,'zh')` 拼音序）/ 问题优先（综合升序）)/ 组件评分详情（`#compDetail`，**正文内容用户待提供，点格联动占位**）。矩阵与详情复用旅程矩阵的 `tpCellHtml/faceIcon/heatColor/miniSparkSvg/tp-d-*` 同一套样式。（注：`touchpointData` 此处是组件评分数据，`sceneName` 字段实为组件名；matrix.html 那套 hccl/dvpp 数据未采用。）
