...
沿用 overview-v3 的 topbar / filter-bar / 网络装饰 / token / 组件。**两页 topbar 互通**：topbar 仅 **2 个 tab——总览 / 产品易用性**（原「体验测试」tab 已去掉，原「用户旅程」tab 已更名为「产品易用性」，仍指向 `journey-v3.html`）。journey 的「总览」tab 跳 `overview-v3.html`，overview 的「产品易用性」tab 跳 `journey-v3.html`（均为 `location.href` 跳转）。filter-bar 此页用 **角色**（算子开发者 / AI 框架开发者 / 应用开发者）= CANN 终端开发者角色，**不是**总览页的「视角」内部使用者 chips，两套别混（见设计约定）。filter-bar 分组顺序为 **芯片 → 角色 → 视图**（视图在末尾），三个 `.fl-lbl` 标签前各带一个线性 SVG icon（芯片=cpu/引脚、角色=用户、视图=四宫格；`.fl-lbl` 设 `inline-flex`，`.fl-lbl svg` 统一 13px 描边图标）。

**本页含两个视图**，由 filter-bar 末尾的「视图」chip（旅程 / 组件）经 `setView('journey'|'component')` 切换（toggle `#view-journey` / `#view-component` 显隐；组件视图首次切入时 `renderCompMatrix()` 懒加载）：
- **旅程视图** `#view-journey`：三楼层 `#f-top`（总览）/ `#f-matrix`（场景体验矩阵）/ `#f-scene`（场景深潜）。
- **组件视图** `#view-component`：综览（同款 `.jkpi` 卡，11 个组件口径）/ 组件体验矩阵（`#compHeatGrid`，**行=11 个组件（算子开发 / Ascend C / 图引擎 / 运行时 / 算子编程-PyPTO / 算子库 / 通信库-集合通信 / 通信库-单边通信 / 工具 / PTO / 编译库&OS），列=7 评分维度（资料/文档·API/接口·源码&示例类·工具·版本·运行反馈·社区）+ 综合**，数据 `touchpointData`（cells 由 `_cc()` 按 7 维生成 `{s,t}`，模拟），`renderCompMatrix`/`selectCompCell` 点格 → `#compDetail`；行排序 `setCompSort('alpha'|'score')`——默认字母（拼音序）/ 问题优先（综合升序））。**统一的两级交互（场景矩阵与组件矩阵一致）**：
     - **点格子 → 当前楼层**展开该格（场景×阶段 / 组件×维度）明细：场景矩阵写 `#tpDetail`（`selectStageCell`），组件矩阵写卡内 `#compDetail`（`selectCompCell` → 头部分数块/7维分值条 + 该维度的子维度环形时间线 `_dimRingTimeline`）。
     - **点行名（场景名 / 组件名，`.rl-click`）→ 下一楼层**展开整场景/整组件：场景名 `selScene` 滚到 `#f-scene` 深潜；组件名 `selectCompRow` 写 `#f-compdeep` 的 `#compDeep`（`renderCompDeep`：det-hd 头 + 7 维分值条 + 按维度的环形时间线，每维度下挂子维度 `_dimGroupsHtml` 的检查项卡）。
   - 评分依据 `SCORE_RUBRIC`（整理自 `reference/组件评分制度.CSV`，定义维度→子维度→检查项→量化指标）只作得分依据、**页面不单独陈列制度**；得分 `_itemScore` 围绕维度分模拟；CSV 仅覆盖 资料文档·API·源码示例 3 维，其余维度显「明细待补充」。
   - **每个维度都带「总结 + 分析」**（与场景页的 总结/观测 对齐）：`_dimAnalysis`/`_dimAnalysisHtml` 由各检查项得分自动生成——总结按维度分分档 + 点名最弱子维度，分析列出得分最低的 3 个检查项；格子明细和整组件深潜都附。
   - **视图 chip 文案**：场景 / 组件（内部 `setView('journey'|'component')`，'journey' 即"场景"视图）。
   - **综合列特殊样式**（区别于维度列）：表头 `.hm-header.sum` **灰底**（`--g2`，避免蓝色像选中）；单元格 `.hm-overall` 数字加粗放大（`tpCellHtml` 中 `!clickAttr` 即综合格）。场景矩阵与组件矩阵的「综合」列均如此。矩阵与详情复用 `tpCellHtml/heatColor/miniSparkSvg/tp-d-*` 样式。（注：`touchpointData` 此处是组件评分数据，`sceneName` 字段实为组件名。）
   - **两个矩阵的通用呈现**：单元格已**去表情脸**（`faceIcon` 不再用）、**行高压缩**（cell/header/row-label 36px、gap 6px）、分数走**十分制**（`fmt10(s)=(s/10).toFixed(1)` 仅用于展示，底层仍 0-100 用于配色）。组件视图首次进入 `compDefaultSelect()` **默认选中**最差组件里有评分制度明细的最差维度（`_compSel` 记录，排序后重渲染会回填高亮）。
   - **场景共 6 个**（`scenarios` id 0-5）：算子复现部署 / Builtin 算子定制修改 / 算子迁移部署 / 算子基本功能实现 / 特定 shape 性能优化 / 泛化 shape 性能优化；`STAGE_UNION` 8 阶段并集（…+ 性能分析 / 性能调优，性能优化类场景用）。
...
2. **场景体验矩阵**（原"组件/触点体验矩阵"，样式/交互照搬 `matrix.html` 重绘）：CSS grid（`#tpHeatGrid`），**行=场景，列=旅程阶段**（`STAGE_UNION` 6 阶段的"大而全"并集 + 综合列），数据取自 `scenarios[].agentSteps`（`renderTpMatrix` 用 `_sceneStage` 取该场景对应阶段的 step）；**某场景没有的阶段渲染 `.hm-empty` 空白格**（虚线、不可点、显"–"）。每格独立**圆角小卡**（48px，淡底 5% + 同色 10% 描边 + 表情脸 `faceIcon` + 数字），数字用 JetBrains Mono。色阶 `heatColor/borderColor/textColor` + hover 版为 **5 段：低→红/橙/绿/青/蓝←高**（matrix.html 方案，情绪靠表情脸传达）。hover 描边 2px+实色，选中 `.hm-sel` 紫环+`scale(1.06)`。点格 `selectStageCell(sceneId,stageIdx)` → **网格下方** `#tpDetail` 内联详情（分数大色块 + 场景·阶段名 + 近 6 期迷你折线 `miniSparkSvg` + 各阶段分值条 + 该阶段 `summary`/`observations`）。综合列不可点。折线无真实历史，由 `tpHist` 按当前分+场景 `trendDir` 合成 6 点。
3. **场景深潜**：场景列表（`.sc` 卡，`renderList()`，问题优先/目录顺序 `setSort()` 切换，`catalogOrder=[0,2,1,3]`）+ 场景详情 **主从联动**（`selScene(id)`，初始 `selScene(0)` 默认展开最差场景）。详情含：
   - **Agentic 指标卡**（`#detKpis.mk-grid`，4 张 `.mk-card`，样式参考 `reference/阶段的指标卡片.jpg`）：开发成功率 / 平均复杂度（tokens消耗量）/ 平均任务执行时长 / 用例执行通过率 ——**替代了 v2.0 的「5 维诊断雷达」**。每卡：左上图标(`KPI_META[i].ic`/accent/soft) + 右上「近7次数据」 / 指标名 / 大数值+单位+占比(`kpiFrac`，成功率 /47、通过率 /300，由百分比合成)+趋势 / 右侧迷你图（`KPI_META[i].chart`：成功率·通过率=`kpiBars` 末柱高亮，复杂度·时长=`kpiLineChart`；7 点序列 `kpiSeries(base,trendCls)` 合成，无真实历史）。数据仍在 `scenarios[].kpis`，元信息按 index 取 `KPI_META`。`.mk-card` **无阴影**、底色用浅灰 `--g1`（让它归属于详情块、不与场景卡同层级）。
   - **Agent 逐步评分**（旅程环节时间线 `.step-tl` + 指标网格）直接平铺展示（「工具调用链」已按用户要求移除，`scenarios[].tools` 数据保留未用）。顶部有 **算子切换** chip（`.op-switch`/`.op-chip`：总体 / add / grid_sample / matmul / softmax / layer_norm），`renderAgentPanel(s,op)` + `selectOp(op)` 重渲染；各算子的逐阶段分由总体分加 `OP_DELTA[op]` 偏移合成（模拟，summary/观测/指标沿用总体），`_clsOf` 按分重算颜色。原 v2.0 的「三类证据 Tab（Agent/痛点/VOD）」已**去掉 tab 切换**——痛点 / VOD 改由证据速览 popover 查看（见下），详情区只剩 Agent 逐步评分。步骤评分用 `.step-tl-dot` 内的**环形进度图**呈现（`stepRing(score,cls)` 内联 SVG，`pathLength=100` + `stroke-dasharray` 映射百分比，中心 `.sd-num` 叠数字），原来的横向 bar 和单独 `as-score` 文字已移除；步骤头只剩 名称 + 右侧「痛点·VOD ›」入口。
   ...
   - 数据模型 `scenarios[]`（id 0-5，后 2 个为性能优化场景，模拟数据）+ `kpis` 字段；**已去掉 v2.0 的「设计点」侧栏**（①–⑥ 设计理由讲解），与 overview-v3 一致保持干净成品。
