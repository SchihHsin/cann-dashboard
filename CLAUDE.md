...
- **组件视图** `#view-component`：综览（同款 `.jkpi` 卡，11 个组件口径）/ 组件体验矩阵（`#compHeatGrid`，**行=11 个组件（算子开发 / Ascend C / 图引擎 / 运行时 / 算子编程-PyPTO / 算子库 / 通信库-集合通信 / 通信库-单边通信 / 工具 / PTO / 编译库&OS），列=7 评分维度（资料/文档·API/接口·源码&示例类·工具·版本·运行反馈·社区）+ 综合**，数据 `touchpointData`（cells 由 `_cc()` 按 7 维生成 `{s,t}`，模拟），`renderCompMatrix`/`selectCompCell` 点格 → `#compDetail`；行排序 `setCompSort('alpha'|'score')`——默认字母（拼音序）/ 问题优先（综合升序）)/ 组件评分详情（`#compDetail`，点格联动占位）/ **组件评分制度**（`#rubSwitch`+`#rubBody`，`SCORE_RUBRIC`/`renderRubric(key)` 整理自 `reference/组件评分制度.CSV`：按维度切换，每维度按子维度分组列「检查项 / 量化指标 / 打分规则」；CSV 仅覆盖 资料文档·API·源码示例 3 维，其余 4 维 chip 标「·待补充」不可点）。
   - **综合列特殊样式**（区别于维度列）：表头 `.hm-header.sum` 蓝填充；单元格 `.hm-overall` 去表情脸、数字加粗放大（`tpCellHtml` 中 `!clickAttr` 即综合格）。旅程场景矩阵与组件矩阵的「综合」列均如此。矩阵与详情复用旅程矩阵的 `tpCellHtml/faceIcon/heatColor/miniSparkSvg/tp-d-*` 样式。（注：`touchpointData` 此处是组件评分数据，`sceneName` 字段实为组件名。）
