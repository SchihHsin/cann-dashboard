...
- **组件视图** `#view-component`：综览（同款 `.jkpi` 卡，11 个组件口径）/ 组件体验矩阵（`#compHeatGrid`，**行=11 个组件（算子开发 / Ascend C / 图引擎 / 运行时 / 算子编程-PyPTO / 算子库 / 通信库-集合通信 / 通信库-单边通信 / 工具 / PTO / 编译库&OS），列=7 评分维度（资料/文档·API/接口·源码&示例类·工具·版本·运行反馈·社区）+ 综合**，数据 `touchpointData`（cells 由 `_cc()` 按 7 维生成 `{s,t}`，模拟），`renderCompMatrix`/`selectCompCell` 点格 → `#compDetail`；行排序 `setCompSort('alpha'|'score')`——默认字母（拼音序）/ 问题优先（综合升序）)/ 组件评分详情（`#compDetail`，点格 `selectCompCell` → 顶部分数块/维度分值条 + **依评分制度的逐项评分结果，呈现方式与场景页一致**：`_compScoreBreakdown(k,base)` 用 `SCORE_RUBRIC`（整理自 `reference/组件评分制度.CSV`）把子维度做成**环形时间线**（复用场景页的 `.step-tl`/`stepRing`/`.step-metrics`）——每个子维度一个环形分节点（分=其检查项均值），检查项做成指标卡（label=量化指标名取 `=` 前部分，value=得分），得分由 `_itemScore` 围绕维度分模拟浮动。**注意：评分制度只是得分依据，页面不单独陈列制度，展示的是结果**；CSV 仅覆盖 资料文档·API·源码示例 3 维，其余维度 `_compScoreBreakdown` 显「明细待补充」）。
