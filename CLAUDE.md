...
- **组件视图** `#view-component`：综览（同款 `.jkpi` 卡，围绕 4 类组件 文档/API/工具/环境）/ 组件体验矩阵（`#compHeatGrid`，**行=场景，列=文档/API/工具/环境 + 综合**，数据 `touchpointData`，`renderCompMatrix`/`selectCompCell` 点格 → `#compDetail` 内联详情，含 obs）/ 组件评分详情（`#compDetail`，**正文内容用户待提供，点格联动占位**）。矩阵与详情复用旅程矩阵的 `tpCellHtml/faceIcon/heatColor/miniSparkSvg/tp-d-*` 同一套样式。（注：matrix.html 那套 hccl/dvpp 组件评分数据本次未采用，用户要的是 文档/API/工具/环境 列。）
