...
**配色约定（用户明确要求）**：主强调色用「偏紫的蓝」(`--purple`/`--purple-dark` #6A67CE/#4F4DA7)——场景详情大号数字（det-score、4 个 Agentic KPI 值、pain-score）、场景卡 rank 徽章、选中态边框都走紫蓝；**红色只保留为小面积状态标记**（grade-tag 严重 / 场景列表 score / 热力图 / 小趋势箭头），不要让红成为页面主题色。**不要用左描边 / 上描边**（左侧/顶部彩色 accent bar）的卡片样式，统一用 1px 整框 + 柔阴影。
