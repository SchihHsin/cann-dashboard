...
锚点 id 依次为 `#f-top / #f-matrix / #f-scene`，沿用 overview-v3 的 topbar / filter-bar / 网络装饰 / token / 组件。topbar「总览」tab 跳 `overview-v3.html`。filter-bar 此页用 **角色**（算子开发者 / AI 框架开发者 / 应用开发者）= CANN 终端开发者角色，**不是**总览页的「视角」内部使用者 chips，两套别混（见设计约定）。
...
3. **场景深潜**：场景列表（`.sc` 卡，`renderList()`，问题优先/目录顺序 `setSort()` 切换，`catalogOrder=[0,2,1,3]`）+ 场景详情 **主从联动**（`selScene(id)`，初始 `selScene(0)` 默认展开最差场景）。详情含：
   ...
   - **排名对照** mini bars（角色内 4 场景柱对比，当前高亮）+ **近 7 次评测趋势**（ECharts line，实例存全局 `_trendChart`，`renderTrend()` 内先 `dispose()` 再 `init` 防重复初始化报错；均值线 label 用 `Math.round` 防小数）
