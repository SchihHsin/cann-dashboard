...
**配色 / 样式约定**：场景评分相关数字、徽章、标签按 grade 走语义色（红 bad / 金 mid / 绿 ok，问题优先）。**不要用左描边 / 上描边**（左侧/顶部彩色 accent bar）的卡片样式，统一用 1px 整框 + 柔阴影。**楼层标题不带彩色 icon 徽章**（`.floor-badge` 已从各楼层移除——用户嫌颜色多），floor-hd 直接以标题起头。**总览楼层（Floor1）4 张 KPI 卡用毛玻璃效果**：`.card.jkpi`（半透明白 + `backdrop-filter:blur(16px) saturate` + 斜向高光 ::before），直接铺在页面浅色底上（用户不要彩色渐变背板）。

1. **总览**：4 张 hero KPI 卡（`.jkpi`）——整体健康评分 / 严重场景 / 活跃痛点 / 本期优先处理（chip 点击锚跳并 `selScene` 展开对应场景）
2. **触点体验矩阵**：纯 CSS grid 热力图（`#tpHeatGrid`，4 场景 × 文档/API/工具/环境 + 综合列），`heatColor()/textColor()`（红→黄→绿，低分=红）着色，点格 `selectTpCell()` → 右侧 `#tpDetail` 面板显示 Agent 观测；默认 `resetTpDetail()` 占位
3. **场景深潜**：场景列表（`.sc` 卡，`renderList()`，问题优先/目录顺序 `setSort()` 切换，`catalogOrder=[0,2,1,3]`）+ 场景详情 **主从联动**（`selScene(id)`，初始 `selScene(0)` 默认展开最差场景）。详情含：
   - **Agentic 指标条**（4 格 `.kpi-row`）：开发成功率 / 平均复杂度 / 平均任务执行时长 / 用例执行通过率 ——**替代了 v2.0 的「5 维诊断雷达」**（因 4 指标单位不同，改用 KPI 指标条而非雷达；数据在 `scenarios[].kpis`）
   - **Agent 逐步评分**（旅程环节时间线 `.step-tl` + 指标网格 + 工具调用链）直接平铺展示。原 v2.0 的「三类证据 Tab（Agent/痛点/VOD）」已**去掉 tab 切换**——痛点 / VOD 改由证据速览 popover 查看（见下），详情区只剩 Agent 逐步评分。
   - 注：v2.0 的「排名对照 mini bars」和「近 7 次评测趋势（ECharts line）」已按用户要求**移除**；`scenarios[].runs` 仅余场景卡 sparkline 使用。
   - **证据速览 popover**（`#evPop`，JS 动态建一个 `.evpop`）：场景卡的「痛点 N ›/VOD N ›」chip 和 Agent 旅程环节步骤卡的「痛点·VOD ›」按钮都是入口——**悬浮预览 / 点击锁定**（`showEvPop(id,anchor,focus,pin)`，focus='pain'|'vod'|null），弹出所属场景的痛点 + VOD 详情（痛点/VOD 是场景级数据，步骤卡弹的是其所属场景）。锁定态点空白/滚动关闭；scene chip 的 onclick 需 `stopPropagation` 以免触发选场景。
