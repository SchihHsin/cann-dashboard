...
沿用 overview-v3 的 topbar / filter-bar / 网络装饰 / token / 组件。**两页 topbar 互通**：journey 的「总览」tab 跳 `overview-v3.html`，overview 的「用户旅程」tab 跳 `journey-v3.html`（均为 `location.href` 跳转，「体验测试」仍是 alert 占位）。filter-bar 此页用 **角色**（算子开发者 / AI 框架开发者 / 应用开发者）= CANN 终端开发者角色，**不是**总览页的「视角」内部使用者 chips，两套别混（见设计约定）。
