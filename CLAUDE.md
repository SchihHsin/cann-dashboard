...
**配色 / 样式约定**：场景评分相关数字、徽章、标签按 grade 走语义色（红 bad / 金 mid / 绿 ok，问题优先）。**不要用左描边 / 上描边**（左侧/顶部彩色 accent bar）的卡片样式，统一用 1px 整框 + 柔阴影。**楼层标题不带彩色 icon 徽章**（`.floor-badge` 已从各楼层移除——用户嫌颜色多），floor-hd 直接以标题起头。**总览楼层（Floor1）4 张 KPI 卡用玻璃折射效果**：外层 `.glass-stage`（紫蓝渐变背板）+ `.card.jkpi`（半透明白 + `backdrop-filter:blur` + 斜向高光 ::before）——折射靠背板渐变透出，所以 glass 卡必须放在有渐变/内容的 stage 上才出效果。
