...
- **组件视图** `#view-component`：综览（同款 `.jkpi` 卡，11 个组件口径）/ 组件体验矩阵（`#compHeatGrid`，**行=11 个组件（算子开发 / Ascend C / 图引擎 / 运行时 / 算子编程-PyPTO / 算子库 / 通信库-集合通信 / 通信库-单边通信 / 工具 / PTO / 编译库&OS），列=7 评分维度（资料/文档·API/接口·源码&示例类·工具·版本·运行反馈·社区）+ 综合**，数据 `touchpointData`（cells 由 `_cc()` 按 7 维生成 `{s,t}`，模拟），`renderCompMatrix`/`selectCompCell` 点格 → `#compDetail`；行排序 `setCompSort('alpha'|'score')`——默认字母（拼音序）/ 问题优先（综合升序））。`#compDetail` **就在组件矩阵的同一张卡内**（grid 下方，和场景矩阵的 `#tpDetail` 同构，不再单独成楼层）；点任意格子 = **选中该组件**（行标 `.rl-sel` 高亮 + 所点格子 `.hm-sel`），`renderCompDetail(sceneId,activeK)` **以组件为维度展开整组件**（不是以单格维度）：顶部 组件综合分块 + 7 维分值条 + 近6期；下方按**评分维度**做环形时间线（复用场景页 `.step-tl/stepRing/.step-metrics`），每个维度一个环形节点（分=该维度矩阵分），节点内按子维度（`SCORE_RUBRIC` 整理自 `reference/组件评分制度.CSV`）列检查项指标卡（label=量化指标名取 `=` 前，value=得分，`_itemScore` 围绕维度分模拟）；所点维度节点名高亮「· 当前」。**评分制度只是得分依据、不单独陈列**；CSV 仅覆盖 资料文档·API·源码示例 3 维，其余维度节点显「明细待补充」。
