...
- **组件视图** `#view-component`：综览（同款 `.jkpi` 卡）/ 组件体验矩阵（`#compHeatGrid`，沿用 `matrix.html` 数据：行=hccl/dvpp/dump/amct/ge，列=总分/测试评分/开发评分/可理解/可操作/根因/完整/一致/准确，`compNorm(ci,v)` 把原始值归一到 0-100 着色与显示）/ 组件评分详情（`#compDetail`，`selectCompCell` 渲染，**正文内容用户待提供，目前为占位**）。矩阵与详情复用旅程矩阵的 `tpCellHtml/faceIcon/heatColor/miniSparkSvg/tp-d-*` 同一套样式。
